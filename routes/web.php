...
use App\Http\Controllers\hrm\EmployeeController; // <-- NEW IMPORT
...
Route::prefix('dashboard/hrm')->name('hrm.')->middleware(['auth', 'verified'])->group(function () {
    ...
    // Manager dashboard (new dashboard with charts)
    Route::get('/manager', [HrmDashboardController::class, 'managerDashboard'])
        ->middleware(['role:HRM', 'position:manager'])
        ->name('manager.dashboard');

    // Employee management (old directory and trainee grading)
    Route::get('/manager/employees', [HrmDashboardController::class, 'employeeManagement'])
        ->middleware(['role:HRM', 'position:manager'])
        ->name('manager.employee');

    // Staff dashboard
    Route::get('/employee', [HrmDashboardController::class, 'staffDashboard'])
    ...
    Route::post('/attendance/toggle', [AttendanceController::class, 'toggle'])->name('employee.attendance.toggle');
    Route::delete('/attendance/shift', [AttendanceController::class, 'destroyShift'])->name('employee.attendance.destroy-shift');
    Route::post('/attendance/shift/remove-monthly', [AttendanceController::class, 'destroyMonthlyShift'])->name('employee.attendance.destroy-monthly-shift');
    ...
    Route::post('/payroll/store', [HrmstaffpayrollController::class, 'store'])->name('hrm.employee.payroll.store');
    Route::patch('/payroll/{id}/approve', [HrmstaffpayrollController::class, 'approve'])->name('employee.payroll.approve');
    Route::patch('/payroll/{id}/reject', [HrmstaffpayrollController::class, 'reject'])->name('employee.payroll.reject');
    Route::post('/holidays/store', [HrmstaffpayrollController::class, 'storeHoliday'])->name('hrm.holidays.store');

    // Manager additional routes
    Route::post('/manager/finalize-promotion/{id}', [HrmDashboardController::class, 'finalizePromotion'])
        ->middleware(['role:HRM', 'position:manager'])
        ->name('manager.finalize-promotion');
    Route::post('/manager/grade-trainee/{id}', [HrmDashboardController::class, 'gradeAndPromote'])
        ->middleware(['role:HRM', 'position:manager'])
    ...
    // Shared Applicant Module
    Route::controller(ApplicantController::class)->group(function () {
        Route::get('/applicants', 'index')->middleware(['role:HRM', 'position:manager,staff'])->name('applicants.index');
        Route::post('/applicants', 'store')->middleware(['role:HRM', 'position:manager,staff'])->name('applicants.store');
        Route::post('/applicants/{applicant}/schedule', 'scheduleInterview')->middleware(['role:HRM', 'position:manager,staff'])->name('applicants.schedule');
        Route::patch('/applicants/{applicant}/update-stage', 'updateStage')->middleware(['role:HRM', 'position:manager,staff'])->name('applicants.update-stage');
        Route::post('/applicants/{applicant}/create-user', 'createUser')->middleware(['role:HRM', 'position:manager,staff'])->name('applicants.create-user');
    });

    // Onboarding
    Route::controller(OnboardingController::class)->group(function () {
        Route::get('/onboarding', 'onboarding')->middleware(['role:HRM', 'position:manager'])->name('manager.onboarding');
    });
});
...
Route::prefix('dashboard/man')->name('man.')->middleware(['auth', 'verified'])->group(function () {
    // Staff dashboard – redirects to the appropriate role-specific dashboard
    ...
    // Staff sub‑role pages (unchanged)
